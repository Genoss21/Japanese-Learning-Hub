document.addEventListener("DOMContentLoaded", () => {
  ...
  function getHiraganaLevel(level) {
    let list = [];
    switch(level){
      case "a": list = hiraganaAll.slice(0,5); break;
      case "ka": list = hiraganaAll.slice(5,10); break;
      case "sa": list = hiraganaAll.slice(10,15); break;
      case "ta": list = hiraganaAll.slice(15,20); break;
      case "na": list = hiraganaAll.slice(20,25); break;
      case "ha": list = hiraganaAll.slice(25,30); break;
      case "ma": list = hiraganaAll.slice(30,35); break;
      case "ya": list = hiraganaAll.slice(35,38); break;
      case "ra": list = hiraganaAll.slice(38,43); break;
      case "wa": list = hiraganaAll.slice(43,46); break;
      case "a_ko": list = hiraganaAll.slice(0,10); break;
      case "sa_to": list = hiraganaAll.slice(10,20); break;
      case "na_ho": list = hiraganaAll.slice(20,30); break;
      case "ma_yo": list = hiraganaAll.slice(30,38); break;
      case "ra_n": list = hiraganaAll.slice(38,46); break;
      case "youon": Object.values(youonGroups).forEach(g => list.push(...g)); break;
      case "all": list = [...hiraganaAll]; Object.values(youonGroups).forEach(g => list.push(...g)); break;
    }
    let targetCount = 30;
    if (["a_ko","sa_to","na_ho","ma_yo","ra_n","youon"].includes(level)) targetCount = 50;
  ...
  function renderQuiz(){
    ...
    shuffleArray(others);
    while(choicesSet.size<4 && others.length>0) choicesSet.add(others.pop());
    ...
    choices.forEach(c=>{
      ...
      btn.textContent = c;
      btn.className = "px-4 py-2 bg-slate-600 rounded hover:bg-indigo-600 transition font-semibold text-lg";
  ...
  chooseLevelBtn.addEventListener("click", ()=> levelModal.classList.remove("hidden"));

  closeLevelModalBtn.addEventListener("click", ()=> levelModal.classList.add("hidden"));
